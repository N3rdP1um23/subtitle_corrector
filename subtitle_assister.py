    ...
    def parse_file(self):
        ...
        if current_operation == 'Remove full uppercase lines':
            ...
            for section_index, section_data in enumerate(self.file_data):
                ...
                if not section_index == (len(self.file_data) - 1) and (not regex.search(r'(\-|\–)$', section_data['text'][-1].strip()) and not regex.search(r'^(\-|\–)', section_data['text'][-1].strip())) and (regex.search(r'(\w|\-|\–)$', section_data['text'][-1].strip()) and regex.search(r'^(\w|\-|\–)', self.file_data[section_index + 1]['text'][0].strip())):
